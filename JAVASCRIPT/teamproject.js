/* =========================================================
   Team Project - LocalStorage MVP (Stable)
========================================================= */
...
function openModal() {
  ...
  // (선택) quiz 결과가 localStorage 어딘가에 있다면 내 역할 기본값으로 세팅
  // 여기서는 보수적으로 USER.job 같은 형태만 사용
  const user = readUser();
...
function seedProjects() {
  const user = readUser();
  const leaderName = user?.name || "Team Member";
  ...
  const samples = [
    {
      ...
      leader: leaderName,
      members: [{ userName: leaderName, role: "FE", isLeader: true, joinedAt: nowISO() }],
    ...
    },
    {
      ...
      tags: ["HTML", "CSS", "JS"],
      leader: leaderName,
      members: [{ userName: leaderName, role: "PM", isLeader: true, joinedAt: nowISO() }],
      neededRoles: ["BE", "PM"],
      createdAt: nowISO(),
      createdBy: leaderName,
    },
...
function normalizeProject(p) {
  return {
    ...
    createdBy: p.createdBy ?? p.leader ?? "Unknown",
  };
}

function isUserInProject(p, user) {
  if (!user) return false;

  const uid = user.id || user.userId || user.email || user.name;
  const uname = user.name;

  return (p.members || []).some(
    (m) =>
      (m.userId && m.userId === uid) ||
      (!m.userId && m.userName === uname) // 기존 데이터 호환
  );
}
...
function render() {
  ...
  // 내 참여 프로젝트만 보기
  if (onlyMyProjects && userName) {
    projects = projects.filter((p) => isUserInProject(p, userName));
  }
  ...
  projects.forEach((p) => {
    ...
    const isLeader = user && (p.leader === user.name);
    const isMember = isUserInProject(p, user);
    ...
    card.innerHTML = `
      ...
      <div class="card-footer">
        ...
        <div class="card-actions">
          ...
          <button class="action-btn join" data-action="join" data-id="${p.id}">참여하기</button>
          <button class="action-btn danger" data-action="delete" data-id="${p.id}">삭제</button>
        </div>
      </div>
    `;
...
grid?.addEventListener("click", (e) => {
  ...
  if (action === "join") {
    const already = isUserInProject(p, user);
    if (already) {
